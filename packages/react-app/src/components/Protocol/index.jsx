...
import { ERC20Interface, useContractFunction, useDebounce, useEthers, useToken, useTokenAllowance, useTokenBalance, useEtherBalance } from '@usedapp/core';
  ...
  return active && token0 && token1 ? (
    <>
      <form className="tag">
        ...
        <div className="labelBox flex flex-row space-x-2 my-2 ">
          <label className="w-40 font-bold ml-2 ">
            <Select
              ...
              value={swapLimit ? value1 : value2}
              placeholder="Select Token"
              onChange={swapLimit ? handleChange1 : handleChange2}
              options={options}
            />
            <br />
        ...
        <button className="rounded-full border border-gray-300 hover:bg-gray-200 w-6 h-6 flex justify-center items-center ml-auto" onClick={onSwapLimit}>
          <RiArrowUpDownLine />
        </button>
        <div className=" labelBox flex flex-row space-x-2 my-2 ">
          <label className="w-40 font-bold ml-3">Price</label>
        ...
        <div className="labelBox flex flex-row space-x-2 my-2 ">
          <label className="w-40 font-bold ml-2">
            <Select
              className="w-30 mt-2"
              styles={selectionStyle}
              value={swapLimit ? value2 : value1}
              placeholder="Select Token"
              onChange={swapLimit ? handleChange2 : handleChange1}
              options={options}
            />
            <br />
